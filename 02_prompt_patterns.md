...
- **Instructions given only in conversation don't survive `/compact`.** Compaction is Claude Code's context compression: when the context window fills, the conversation so far is replaced by a summary - automatically, or when you type `/compact`. The project-root CLAUDE.md is re-read from disk afterward; a rule you typed in chat survives only as whatever the summary kept. Another argument for files over chat.
