...
- Path-scoped rules live in `.claude/rules/` and load automatically when a session touches files matching their `paths:` globs - `status_conduct.md` applies under `.status/`. Nothing needs to reference them; this note exists so a reader knows they are there.
